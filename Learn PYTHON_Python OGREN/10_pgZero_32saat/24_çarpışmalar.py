...
new_image = 'uzaylı_uzaylı' # Anlık Görüntü
...
def update(dt):
    ...
    if uzayli.colliderect(kutu): # Çarpışma 
        if new_image != 'uzaylı_yaralı':
            uzayli.image = 'uzaylı_yaralı'
            new_image = 'uzaylı_yaralı'
        
def on_key_down(key):
    if keyboard.space or keyboard.up or keyboard.w:# Zıplama
        uzayli.y = 100

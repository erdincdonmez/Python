...
#! MODEL TUNİNG
denenecek_parametreler = {"learning_rate":[.1,.01,.03],
                          "iterations":[200,500,1000], # Ağaç sayısı
                          "depth":[4,5,8]}

catb = CatBoostClassifier()
catb_cv_model = GridSearchCV(catb, denenecek_parametreler, cv=5, n_jobs=-1, verbose=2).fit(X_train, y_train)

print("\n\ncatb_cv_model.get_params(): \n",catb_cv_model.get_params())
print("\n\ncatb_cv_model.best_params_ : \n",catb_cv_model.best_params_)

#! FİNAL MODELİ
catb_tuned = CatBoostClassifier(max_depth=1, learning_rate=.001,n_estimators=500).fit(X_train, y_train)

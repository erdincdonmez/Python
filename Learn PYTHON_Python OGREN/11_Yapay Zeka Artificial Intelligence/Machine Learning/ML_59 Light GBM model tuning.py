...
lgbm = LGBMClassifier()
lgbm_cv_model = GridSearchCV(lgbm, denenecek_parametreler, cv=10, n_jobs=-1, verbose=2).fit(X_train, y_train)

print("\n\nlgbm_cv_model.get_params(): \n",lgbm_cv_model.get_params())
print("\n\nlgbm_cv_model.best_params_ : \n",lgbm_cv_model.best_params_)

#! FİNAL MODELİ
lgbm_tuned = LGBMClassifier(max_depth=1, learning_rate=.01,n_estimators=500).fit(X_train, y_train)

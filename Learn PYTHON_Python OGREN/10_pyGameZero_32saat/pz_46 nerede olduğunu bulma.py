...
def draw():
    ...
    if mod == 'oyun':# Oyun Modu
        uzay.draw()
        gezegenler[0].draw()
        for i in range(len(meteorlar)):# Meteorların Çizilmesi
            meteorlar[i].draw()
        gemi.draw()
        for i in range(len(dusmanlar)):# Düşmanların Çizilmesi
            dusmanlar[i].draw()
...
def yeni_dusman():# Yeni Düşmanların Eklenmesi
...
def gezegen():# Gezegenlerin Hareketleri
